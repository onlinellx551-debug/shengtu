...
def paste_with_shadow(canvas: Image.Image, image: Image.Image, x: int, y: int, blur: int = 14, alpha: float = 0.22) -> None:
...
def compose_stack(dest: Path, assets_dir: Path) -> None:
    canvas = Image.new("RGBA", (900, 900), (214, 178, 149, 255))
    white = extract_subject(assets_dir / "tail_03_white_flat.jpg")
    gray = extract_subject(assets_dir / "tail_04_gray_flat.jpg")
    black = extract_subject(assets_dir / "tail_02_black_flat.jpg")
    ivory = tint_subject(gray, (198, 188, 174), (244, 238, 229))
    olive = tint_subject(gray, (93, 99, 78), (132, 140, 108))
    srcs = [ivory, white, gray, olive, black]
    positions = [(110, 258), (190, 214), (278, 176), (364, 142), (454, 112)]
    scales = [0.86, 0.88, 0.90, 0.91, 0.92]
    for subject, pos, scale in zip(srcs, positions, scales):
        subject = subject.resize((int(subject.width * scale), int(subject.height * scale)), Image.Resampling.LANCZOS)
        paste_with_shadow(canvas, subject, pos[0], pos[1], blur=12, alpha=0.18)
    canvas.convert("RGB").save(dest, quality=95)


def compose_scatter(dest: Path, assets_dir: Path) -> None:
    canvas = Image.new("RGBA", (900, 900), (214, 178, 149, 255))
    white = extract_subject(assets_dir / "tail_03_white_flat.jpg")
    gray = extract_subject(assets_dir / "tail_04_gray_flat.jpg")
    black = extract_subject(assets_dir / "tail_02_black_flat.jpg")
    ivory = tint_subject(gray, (198, 188, 174), (244, 238, 229))
    srcs = [ivory, white, gray, black]
    rotations = [-11, -7, -3, 8]
    positions = [(92, 208), (182, 148), (278, 106), (390, 78)]
    scales = [0.82, 0.86, 0.88, 0.92]
    for subject, angle, pos, scale in zip(srcs, rotations, positions, scales):
        subject = subject.resize((int(subject.width * scale), int(subject.height * scale)), Image.Resampling.LANCZOS)
        subject = subject.rotate(angle, expand=True, resample=Image.Resampling.BICUBIC)
        paste_with_shadow(canvas, subject, pos[0], pos[1], blur=16, alpha=0.22)
    canvas.convert("RGB").save(dest, quality=95)
...
def ensure_round4_assets(web_preview_dir: Path) -> None:
    assets_dir = web_preview_dir / "assets"
    assets_dir.mkdir(parents=True, exist_ok=True)

    # Base images copied into ASCII-safe paths.
    ...
    save_cover_copy(pick(ROUND1_DIR, "G09_*.jpg"), assets_dir / "tail_01_cuff.jpg")
    ...
    # Improved stack/scatter images for round4.
    compose_stack(assets_dir / "top_05_stack_round4.jpg", assets_dir)
    compose_scatter(assets_dir / "top_07_scatter_round4.jpg", assets_dir)

    # Benefit cards closer to the reference section logic.
    build_ref_icon_card(
        source=pick(ROUND1_DIR, "G13_*.jpg"),
        dest=assets_dir / "benefit_neck.jpg",
        headline="领口稳定",
        subline="洗后不易松垮\n日常机洗也更省心",
        icon_label="领",
    )
    build_ref_icon_card(
        ...
        dest=assets_dir / "benefit_soft.jpg",
        headline="柔软肌理",
        subline="贴肤不扎\n通勤和内搭都顺手",
        icon_label="柔",
    )
...
def sync_generated_images() -> None:
    source_dir = ROUND3_DIR / "01_generated"
    target_dir = ROUND4_DIR / "01_generated"
...
def build_html() -> str:
    ...
    top_gallery = [
        ...
        f"{a}/top_05_stack_round4.jpg",
        f"{a}/top_06_fold.jpg",
        f"{a}/top_07_scatter_round4.jpg",
        f"{a}/top_02_collar.jpg",
    ]
    tail_gallery = [
...
{image_tags(top_gallery)}
      ...
      <aside class="buy">
        ...
        <div class="thumbs">
          ...
          <img src="{a}/top_05_stack_round4.jpg" alt="thumb3">
        </div>
...
{image_tags(tail_gallery)}
    ...
    <section class="section">
      <div class="benefit-grid">
        <article class="benefit-card">
          ...
          <div class="benefit-desc">左侧继续保留生活方式位，对应参照页“上身图先讲穿着氛围”的逻辑。</div>
        </article>
        <article class="benefit-card">
          <img src="{a}/benefit_neck.jpg" alt="卖点2">
          <div class="benefit-title">领口稳定，不易松垮</div>
          <div class="benefit-desc">中间位继续讲结构稳定性，贴合用户真实评论里最常见的担心点。</div>
        </article>
        <article class="benefit-card">
          <img src="{a}/benefit_soft.jpg" alt="卖点3">
          <div class="benefit-title">柔软肌理，机洗也省心</div>
          <div class="benefit-desc">右侧位对应参照页的“功能图”，用一张生成卖点图收拢舒适和好打理。</div>
        </article>
      </div>
    </section>
    ...
    <div class="footer">Oakln 商品页结构复排预览 · T01 白T · Round4</div>
  </main>
</body>
</html>
"""
...
- 顶部第 3 排和第 4 排的主图，改成了更像参照页的堆叠/散放构图，不再使用带卡片感的旧草图。
- 中段三联卖点继续保留“生活方式图 + 2 张功能卖点图”的逻辑。
- 员工服装区保持 3 张整身图，继续沿着参照页的留白和卡片布局去靠。

网页文件：

- `04_web_preview/index.html`
- `04_web_preview/preview_check.png`
- `04_web_preview/preview_full.png`
- `04_web_preview/整页长图.png`
"""
    (ROUND4_DIR / "README.md").write_text(text, encoding="utf-8")


def main() -> None:
    sync_generated_images()
    web_preview_dir = ROUND4_DIR / "04_web_preview"
    web_preview_dir.mkdir(parents=True, exist_ok=True)
    ensure_round4_assets(web_preview_dir)
    (web_preview_dir / "index.html").write_text(build_html(), encoding="utf-8")
    screenshot(web_preview_dir / "index.html")
    write_readme()
