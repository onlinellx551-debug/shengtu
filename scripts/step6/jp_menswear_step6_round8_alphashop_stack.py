...
GEN_DIR = ROUND8_DIR / "01_generated"
COMPARE_DIR = ROUND8_DIR / "02_compare"
DEBUG_DIR = ROUND8_DIR / "03_debug"
PREVIEW_DIR = ROUND8_DIR / "04_web_preview"
STAGE_DIR = ROUND8_DIR / "05_stage"
PROFILE_DIR = ROOT / "browser_profiles" / "taobao_1688_manual"
EXPORT_XLSX = ROUND8_DIR / "T01_round8_alphashop_stack_2026-03-24.xlsx"
EXPORT_MD = ROUND8_DIR / "T01_round8_alphashop_stack_2026-03-24.md"

for folder in [ROUND8_DIR, GEN_DIR, COMPARE_DIR, DEBUG_DIR, PREVIEW_DIR, STAGE_DIR]:
...
JOBS: list[Job] = [
    Job("G05A", "顶部第三排第一张 堆叠图", 6, "目标是更像参照页的折叠堆叠服装镜头。"),
    Job("G07A", "顶部第四排第一张 散放图", 8, "目标是更像参照页的散放叠层镜头。"),
]
...
def stage_files(hero_src: Path, ref_map: dict[int, Path]) -> tuple[Path, dict[str, Path]]:
    ...
    for job in JOBS:
        src = ref_map[job.ref_index]
        dst = STAGE_DIR / f"{job.gid}_ref{src.suffix.lower()}"
        ...
        cards.append(
            ...
            <section class="card">
              ...
              <div class="grid">
                ...
                <figure><img src="{cmp_rel}" alt="{row['gid']} compare"><figcaption>参考对比</figcaption></figure>
              </div>
            </section>
            """
        )
...
<head>
  ...
  <title>T01 Round8 AlphaShop 堆叠镜头</title>
  <style>
    body {{ margin:0; font-family:"Microsoft YaHei","PingFang SC",sans-serif; background:#f6f4ef; color:#1d1d1f; }}
    .wrap {{ width:min(1400px, calc(100vw - 48px)); margin:0 auto; padding:32px 0 56px; }}
    ...
    .grid {{ margin-top:16px; display:grid; grid-template-columns:repeat(2, minmax(0, 1fr)); gap:18px; }}
...
<body>
  <div class="wrap">
    <h1>T01 Round8 AlphaShop 堆叠/散放镜头</h1>
    <p class="lead">这一轮只针对顶部主图区的两张高优先级镜头，目标是把“堆叠图”和“散放图”从本地拼图升级成生成镜头。</p>
    {''.join(cards)}
  </div>
</body>
</html>
...
def wait_for_finish(page, timeout_s: int = 180) -> dict:
    ...
    return page.evaluate(
        """() => ({
          ...
          buttons: Array.from(document.querySelectorAll('button')).map((el,i)=>({i,text:(el.innerText||'').trim(),disabled:el.disabled})).filter(x=>x.text)
        })"""
    )
