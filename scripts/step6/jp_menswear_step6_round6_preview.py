...
def extract_subject(path: Path, threshold: int = 30) -> Image.Image:
...
def paste_with_shadow(canvas: Image.Image, image: Image.Image, x: int, y: int, blur: int = 10, alpha: float = 0.16) -> None:
...
def compose_fold_stack(dest: Path, assets_dir: Path) -> None:
    ...
    variants = [
        tint_subject(fold, (200, 190, 176), (246, 241, 234)),
        tint_subject(fold, (236, 236, 236), (252, 252, 252)),
        tint_subject(fold, (160, 160, 160), (214, 214, 214)),
        tint_subject(fold, (18, 18, 18), (74, 74, 74)),
    ]
    canvas = Image.new("RGBA", (900, 900), (214, 178, 149, 255))
    positions = [(36, 315), (145, 258), (268, 207), (395, 165)]
    scales = [0.72, 0.77, 0.82, 0.87]
    for variant, pos, scale in zip(variants, positions, scales):
        ...
        paste_with_shadow(canvas, item, pos[0], pos[1], blur=11, alpha=0.15)
...
def compose_fold_scatter(dest: Path, assets_dir: Path) -> None:
    ...
    canvas = Image.new("RGBA", (900, 900), (214, 178, 149, 255))
    rotations = [-9, -5, -1, 5]
    positions = [(88, 284), (192, 232), (314, 187), (448, 155)]
    scales = [0.68, 0.74, 0.79, 0.84]
    for variant, angle, pos, scale in zip(variants, rotations, positions, scales):
        ...
        paste_with_shadow(canvas, item, pos[0], pos[1], blur=12, alpha=0.15)
    canvas.convert("RGB").save(dest, quality=95)
...
def prepare_round6() -> None:
    web_preview = ROUND6_DIR / "04_web_preview"
    assets_dir = web_preview / "assets"
    generated_dir = ROUND6_DIR / "01_generated"
    generated_dir.mkdir(parents=True, exist_ok=True)
    assets_dir.mkdir(parents=True, exist_ok=True)

    for path in (ROUND5_DIR / "01_generated").glob("*.png"):
        shutil.copy2(path, generated_dir / path.name)
    for path in (ROUND5_DIR / "04_web_preview" / "assets").glob("*"):
        if path.is_file():
            shutil.copy2(path, assets_dir / path.name)

    compose_fold_stack(assets_dir / "top_05_stack_round6.jpg", assets_dir)
    compose_fold_scatter(assets_dir / "top_07_scatter_round6.jpg", assets_dir)

    build_ref_icon_card(
        source=assets_dir / "top_02_collar.jpg",
        dest=assets_dir / "benefit_neck_round6.jpg",
        headline="领口稳定",
        subline="洗后不易松垮\n日常机洗也更省心",
        icon_label="领",
    )
    build_ref_icon_card(
        ...
        dest=assets_dir / "benefit_soft_round6.jpg",
        headline="柔软肌理",
        subline="贴肤不扎\n通勤和内搭都顺手",
        icon_label="柔",
    )
...
def build_html() -> str:
    ...
    top_gallery = [
        ...
        f"{a}/top_05_stack_round6.jpg",
        f"{a}/top_06_fold.jpg",
        f"{a}/top_07_scatter_round6.jpg",
        f"{a}/top_02_collar.jpg",
    ]
    tail_gallery = [
...
{image_tags(top_gallery)}
      ...
      <aside class="buy">
        ...
        <div class="meta">
          Round6 修复了上一轮折叠图发糊的问题。<br>
          本轮目标是让第 3/4 排更接近参照页的“折叠衣物镜头”。
        </div>
        ...
        <div class="thumbs"><img src="{g}/G02.png" alt="thumb1"><img src="{g}/G03.png" alt="thumb2"><img src="{a}/top_05_stack_round6.jpg" alt="thumb3"></div>
...
{image_tags(tail_gallery)}
    ...
    <div class="footer">Oakln 商品页结构复排预览 · T01 白T · Round6</div>
  </main>
</body>
</html>
"""
...
- 修复了上一轮折叠图发糊的问题，改成更锐利的折叠衣物镜头。
- 继续保持整页网页预览、检查图和长图。
- 当前仍未动模特图，后续如果继续提升，优先会转向更接近参照页的生活方式镜头和员工服装裁切。
"""
    (ROUND6_DIR / "README.md").write_text(text, encoding="utf-8")
