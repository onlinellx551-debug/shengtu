...
description: 用于统一驱动日本男装 6 步流程；当需要从任意一步启动、自动判断交接、复用已有输出并在 Step 5 停点后继续到素材包交付时使用。若用户明确指定“只做第六步 / 只做素材包 / 基于链接直接做素材包”，则直接进入 Step 6，不补跑前五步，除非用户明确要求完整流程。
---
...
特殊说明：
- 如果用户明确说“只做第六步 / 只做素材包 / 基于这个链接直接出素材包”，默认进入 **Step 6 直链模式**
- 这时不要补跑 Step 1 到 Step 5
- 只允许把前面步骤的现有输出当作可选参考和加速来源，不允许把“复用旧文件”伪装成“重新完成了前五步”
- 如果用户只想看第六步结果，就不要在回复里长篇叙述前五步历史
...
如果用户只要求第六步且给了链接：
1. 直接进入 `$jp-menswear-step-6-material-pack`
2. 先从链接抓取信息
3. 再把 `step4_output / step5_output / step6_output` 当作可选补充来源
4. 不自动回补 Step 1 到 Step 5
5. 不把“读取旧文件”写成冗长过程说明
...
- Step 6 在已确认同款模式下只使用 Step 5 已确认同款
- Step 6 在直链模式下必须区分哪些信息来自链接、哪些来自历史输出、哪些是推断
...
- 直接进入 Step 6 直链模式
- 先从用户给的链接抽取信息
- 再把 `step4_output / step5_output / step6_output` 当作可选补充来源
- 不自动回补 Step 1 到 Step 5
- 只有在用户明确要求“按完整六步重跑”时，才回退去补前面步骤
